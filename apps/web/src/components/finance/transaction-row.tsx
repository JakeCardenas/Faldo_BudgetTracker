...
export function TransactionRow({ transaction: t, onClick, showDate }: { transaction: Transaction; onClick?: () => void; showDate?: boolean }) {
  ...
  const subtitle = isTransfer ? t.notes ?? "Transfer" : isOwed ? `Money owed · ${t.account_name}`
    : `${t.subcategory_name ?? t.category_name ?? "Uncategorized"} · ${t.account_name}`
  ...
  return (
    ...
    >
      ...
      <span className="shrink-0 text-right">
        <span className={cn("tabular block text-[0.9375rem] font-semibold", t.type === "income" && "text-income", (isTransfer || isOwed) && "font-medium text-muted-foreground")}>
          {amount}
        </span>
