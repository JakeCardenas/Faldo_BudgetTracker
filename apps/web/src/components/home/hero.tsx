...
function FaldoBubble({ data }: { data: Dashboard }) {
  ...
  return (
    <div className="relative w-full rounded-2xl bg-card p-4 text-card-foreground shadow-[0_18px_40px_-20px_rgb(0_0_0/0.5)]">
...
/**
 * The Home centrepiece: what you have and how it has moved, in Faldo's green bamboo environment, with
...
export function BalanceHero({ data }: { data: Dashboard }) {
  ...
  return (
    <section aria-labelledby="balance-title" style={environmentStyle(me?.settings.home_background)}
      ...
      <div className="mt-2 lg:mt-0 lg:grid lg:grid-cols-[minmax(0,1fr)_16rem] lg:gap-10 xl:grid-cols-[minmax(0,1fr)_19rem] xl:gap-14">
        <div className="min-w-0">
          ...
          {/* Phones: Faldo stands beside the balance, below the greeting and above the line, never over it. */}
          <div className="mt-1 grid grid-cols-[minmax(0,1fr)_auto] items-end gap-x-2 lg:mt-8 lg:block">
            ...
            <Panda pose={pose} priority sizes="(min-width: 768px) 200px, (min-width: 640px) 176px, 152px"
              className="pointer-events-none -mr-2 w-[8.75rem] drop-shadow-[0_14px_22px_rgb(0_0_0/0.25)] min-[390px]:w-[9.5rem] sm:w-[11rem] md:w-[12.5rem] lg:hidden" />
          </div>

          <div className="mt-3 h-[10rem] sm:h-48 lg:mt-5 lg:h-60">
            {isLoading ? <Skeleton className="h-full rounded-2xl bg-white/10" /> : series.length > 1 ? (
              <BalanceLine data={series} tone="light" onHover={setHover} height="100%" />
            ) : (
              <p className="flex h-full items-center justify-center rounded-2xl border border-dashed border-white/20 px-6 text-center text-sm text-white/75">Your balance line appears after a few days of activity.</p>
            )}
          </div>
          <div className="mt-3 flex justify-between gap-1 rounded-full bg-white/10 p-1 sm:inline-flex sm:justify-start" role="radiogroup" aria-label="Chart range">
            {RANGES.map((r) => (
        ...
        <aside className="hidden flex-col items-center justify-end gap-4 lg:flex" aria-label="Faldo">
          ...
          <Panda pose={pose} priority sizes="(min-width: 1280px) 288px, 240px" className="w-60 drop-shadow-[0_22px_32px_rgb(0_0_0/0.28)] xl:w-72" />
        </aside>
      </div>
    </section>
  )
