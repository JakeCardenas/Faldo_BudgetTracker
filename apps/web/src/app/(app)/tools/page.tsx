...
export default function ToolsPage() {
  return (
    <div className="space-y-5">
      <LargeTitle title="Tools" subtitle="Calculators and helpers for everyday money" back={{ href: "/", label: "Home" }} />
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {TOOLS.map((tool) => {
          const Icon = tool.icon
          return (
            <Link key={tool.slug} href={`/tools/${tool.slug}`} className="card-surface pressable flex items-start gap-3.5 p-4 hover:border-input">
