...
Faldo is organised around four places: **Home** (money you have now), **Activity** (money in the past), **Plans** (money ahead) and **You** (accounts, tools, learning and settings). On phones they sit in a floating, translucent tab bar with the **+** in the middle; on desktop the same four places sit in a slim top bar, with search, notifications and Add beside them. The design is calm and list-first: colour is kept for Safe to Spend and account cards, the glass material is only used for floating controls, and every number comes from the backend. It supports light and dark mode and can be installed from the browser (Add to Home Screen on iPhone, Install app on Android and desktop).

- **Home:** a green environment with Faldo the panda beside your total balance and a balance line (1W, 1M, 3M, 6M, 1Y; touch it to see a day's balance). Below it: **Safe to Spend** as a calm status (the amount, a per-day pace, what's left this week and a "Why this number?" sheet with the engine's calculation), this month's spending by category, account cards, what's coming up (overdue bills and unconfirmed income first, with Mark paid in the row), one note from Faldo and recent activity.
- **Hide amounts:** an eye button on Home and Accounts (and a switch in Settings) shows every amount as ₱•••• on that device.
...
- **Adding an account:** a three-step sheet with no sideways scrolling: choose a type (cash, e-wallet, bank, savings, debit card, credit card, something else), choose the bank or e-wallet from a searchable list, then name, balance, optional last four digits and colour with a live card preview. Providers carry their brand colours; official logos can be added as files (see `docs/DESIGN_SYSTEM.md`), and none are drawn or generated.
- **Accounts:** net worth, assets or liabilities with a balance line over 1W to 1Y, then account cards in one ordered grid (press and hold to rearrange) or a list grouped by type with totals. Cards use the account's colour; savings and set-aside money get a lighter striped card so it doesn't read as spending money. Cards and bank accounts can show the last four digits if you add them, and nothing more: Faldo never asks for full card numbers, CVVs, PINs or banking passwords. Ready-made templates cover common PH banks and e-wallets.
- **Plans:** "Can I afford it?" (Faldo Check inline), the Money Plan split, bills and income coming up in the next 30 days, planned purchases, goals, budgets, money owed, and links to the forecast, what-if and installments.
- **You:** profile, accounts, statement import, statistics, Talk to Faldo, insights, lessons, streaks and rewards, tools and settings.
- **Faldo Check ("Can I afford it?"):** enter a price (and optionally what it is and its category) from Home, Plans or the + menu. Faldo shows a verdict, Safe to Spend now and after, this week's share, the Money Plan bucket it comes from, budget impact, what's already set aside before your next income, and an estimated goal delay in days. It is pure arithmetic on your data; the decision stays yours. Save it for later or log it if you buy it.
