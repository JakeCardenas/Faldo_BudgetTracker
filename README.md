...
- **Home:** answers five questions and stops. Total balance; **Safe to Spend** on its own green card (the amount, a per-day pace, what's left this week and a "Why this number?" sheet with the engine's calculation); account cards; one note from Faldo (a shortfall or budget warning when there is one, otherwise a short read of the month); what's coming up, with overdue bills and unconfirmed income first and Mark paid right in the row; recent activity; and the goal closest to done.
...
- **Streaks & rewards:** daily logging streak with monthly restores, streak and milestone badges, and unlockable mascot outfits and backgrounds (shown with Faldo's note on Home).
