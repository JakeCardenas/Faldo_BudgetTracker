...
- **Wallet:** net worth, assets or liabilities with a balance line over 1W to 1Y, then real-proportion account cards or a list grouped by type with totals. Reorder opens a simple list with up and down buttons. Cards use the provider's card style or the account's colour; savings and set-aside money get a lighter striped card so it doesn't read as spending money. Cards and bank accounts can show the last four digits if you add them, and nothing more: Faldo never asks for full card numbers, CVVs, PINs or banking passwords. Ready-made templates cover common PH banks and e-wallets.
- **Plan:** "Can I afford it?" (Faldo Check inline), the Money Plan split, bills and income coming up in the next 30 days, planned purchases, goals, budgets, money owed, and links to the forecast, what-if and installments.
- **History:** every transaction by day with search and filters, statistics and insights.
