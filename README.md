...
- **Streaks & poses:** daily logging streak with monthly restores, and Faldo poses unlocked by streaks and first steps (a question, a savings goal, lessons), the chosen one shown on Home.
...
- **Receipt scanning:** any money document, not only store receipts: official receipts and invoices, restaurant bills, Shopee and Lazada orders, GCash, Maya and bank app screenshots (sent, received, bills, InstaPay), utility and phone bills, fuel slips, ride and delivery receipts, ATM slips, payslips and handwritten receipts. On phones the scanner opens straight away: the live camera full screen with corner brackets (like the iPhone's code scanner), a shutter for receipts, Photos for screenshots and the flashlight where the phone allows it. It also reads QR codes as they come into view: a QR Ph payment code (GCash, Maya, banks; checksum verified, `src/lib/qrph.ts`) starts an expense with the store and any amount filled in and the payment method set to QR Ph, and any other code shows what it says, links with their site named. Upload/camera → EXIF-stripped re-encoded image → vision extraction (document type, expense or income, merchant or person, date, items, discounts, fees, tax, total, reference number, due date, paying wallet, bank or card) → validation (items against the total after discounts and fees, dates read the Philippine way with day/month swaps flagged, currency, legibility) → matching (account by card digits, wallet or bank name as whole words, or cash; category or subcategory for that direction) → review form, prefilled → transaction → RAG indexing. A failed scan can be read again. Receipts sent in chat are read too, and Faldo offers a confirm card to log them. Without a vision provider the upload is stored and the UI honestly asks for manual entry.
...
- **Settings:** preferences (timezone, pay frequency, buffer, default account), security (password, signed-in devices), AI memory notes, category editing and deletion, data export, backup and restore, account deletion.
- **Backup and restore:** Settings > Your data downloads a versioned backup (`faldo-backup`, version 1) of every money record, with receipt images (or without, for a smaller file). Restoring shows an exact preview first (it runs the restore inside a savepoint and undoes it), then asks for confirmation tied to that file's checksum. A restore only adds: records the account already has, by id or by name (categories, merchants, tags, budget months, statement lines), are left alone, so restoring twice changes nothing; an account whose name is taken comes back as "Name (restored)". It runs in one transaction, so a failure changes nothing, and receipt images are checked against their checksums and re-processed like an upload. Settings, Faldo's memory (rebuilt afterwards), chats, insights, streaks and import history aren't included. Backups from a newer Faldo, in another currency, or with broken references are refused with a reason.
...
**Staying on free AI:** with `AI_PROVIDER=gemini`, Gemini answers first and **Groq** (`GROQ_API_KEY`, a free tier of open models) backs it up; a paid provider never steps in unless chosen. Free limits are per model, so every job has a list: chats and receipts go to `GEMINI_CHAT_MODEL` then `GEMINI_FALLBACK_MODELS`, short notes to `GEMINI_FAST_MODEL` first, and on Groq, photos go to its vision models. A model that runs out waits as long as the service says (seconds for a per-minute limit, an hour for a daily one) while the next answers; a model the service no longer has is skipped. Only when every model is out does the provider rest and the next one answer. A receipt always goes to a model that can read it, even while the chat rests one. The Reports summary is kept until the data changes, and the Home pulse is reworded by the AI at most once an hour (in between, its template keeps the numbers current), so the free limits go to chats and receipts.
...
```bash
make test        # 186 backend tests, plus the web app's (QR Ph codes)
