...
- **Home:** a green bamboo environment with Faldo the panda beside your total balance and a balance line with a quiet scale (1W, 1M, 3M, 6M, 1Y; drag across it to see a day's balance). Below it: **Safe to Spend** as a calm status (the amount, a per-day pace, what's left this week and a "Why this number?" sheet with the engine's calculation), this month's spending by category, account cards, what's coming up (overdue bills and unconfirmed income first, with Mark paid in the row), one note from Faldo and recent activity.
